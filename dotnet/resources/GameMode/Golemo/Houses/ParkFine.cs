    ...
    class ParkManager : Script
    {
        ...
        internal class ParkFine
        {
            ...
            internal void SpawnPlayerVehicle(Player player, string name, string number)
            {
                ...
                {
                    Log.Write($"A non-existent player tried to call a car in a parking lot {ID} with number {number}", nLog.Type.Error);
                    return;
                }
                if(!Vehicles.ContainsKey(number))
                {
                    Log.Write($"Игрок {Main.Players[player].UUID} tried to call a non-existent car {number}", nLog.Type.Error);
                    return;
                }
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"you called {name} parking space: №{spawnIndex + 1}", 3000);
                ...
                case "evac":
                    ...
                    foreach (var v in NAPI.Pools.GetAllVehicles())
                    {
                        if (v.HasData("ACCESS") && v.GetData<string>("ACCESS") == "PERSONAL" && NAPI.Vehicle.GetVehicleNumberPlate(v) == vehicle)
                        {
                            ...
                            Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"Your car has been parked", 3000);
                            break;
                        }
                    }
                    return;
