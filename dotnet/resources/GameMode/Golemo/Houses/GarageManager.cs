...
namespace Golemo.Houses
{
    ...
    class Garage
    {
        ...
        public void GetVehicleFromGarage(Player player, string number)
        {
            ...
            VehicleManager.ApplyCustomization(veh);
            NAPI.Task.Run(() => { try { player.SetIntoVehicle(veh, 0); } catch { } }, delayTime: 50);
        }
    ...
    class GarageManager : Script
    {
            ...
            try
            {
                switch (id)
                {
                    case 40:
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Вы не можете зайти в гараж с этой машиной", 3000);
                            return;
                        }
        ...
        public static void CMD_SetGarage(Player player, int ID)
        {
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Вы должны быть на разметке дома", 3000);
                return;
            }
        ...
        public static void CMD_CreateGarage(Player player, int type)
        {
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Ты должен сесть в машину!", 3000);
                return;
            }
        ...
        public static void CMD_RemoveGarage(Player player)
        {
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Вы должны быть на маркере гаража", 3000);
                return;
            }
