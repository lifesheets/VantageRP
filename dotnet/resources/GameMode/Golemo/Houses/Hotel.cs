...
namespace Golemo.Houses
{
    class Hotel : Script
    {
        ...
        public static void OpenHotelBuyMenu(Player player)
        {
            ...
            menuItem.Text = $"The rent money will be withdraw every payday only when you are in the game.";
            menu.Add(menuItem);
        ...
        private static void callback_hotelbuy(Player player, Menu menu, Menu.Item item, string eventName, dynamic data)
        {
            switch (item.ID)
            {
                case "rent":
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "You live in the house and cannot rent a room at the hotel", 3000);
                        return;
                    }
                    ...
                    Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"You rented a room at 1 hour.You can extend rent on the phone (m)", 3000);
                    SendToRoom(player);
        ...
        public static void OpenHotelManageMenu(Player player)
        {
            ...
            menuItem.Text = "Extend the rent";
            menu.Add(menuItem);

            menuItem = new Menu.Item("moveout", Menu.MenuItem.Button);
            menuItem.Text = "Take out";
            menu.Add(menuItem);
        ...
        private static void callback_hotelmanage(Player player, Menu menu, Menu.Item item, string eventName, dynamic data)
        {
            switch (item.ID)
            {
                ...
                case "moveout":
                    ...
                    Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, "You evicted from the hotel", 3000);
                    MenuManager.Close(player);
                    return;
                case "close":
                    MenuManager.Close(player);
