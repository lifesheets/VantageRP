...
namespace Golemo.Houses
{
    ...
    class House
    {
        ...
        public void SetOwner(Player player)
        {
            ...
            foreach (var r in Roommates)
            {
                ...
                if (roommate != null)
                {
                    Notify.Send(roommate, NotifyType.Warning, NotifyPosition.BottomCenter, "You have been evicted from your home", 3000);
    ...
    class HouseManager : Script
    {
        ...
        {
            // name, position
            new HouseType("Motorhome", new Vector3(1973.124, 3816.065, 32.30873), new Vector3(), 0.0f, "trevorstrailer"),
                ...
                case 6:
                    {
                        ...
                        if (data != null)
                        {
                            if (data.House == house.ID && data.TypeHouse == house.Type)
                            {
                                ...
                                {
                                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You cannot return to the house with part of its interior..", 3000);
                                    return;
                                }
                                if (house.Locked)
                                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"The house is closed, break it with a lockpick", 3000);
                                else
        ...
        public static void GoHouseMenuInform(Player player, int act)
        {
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"The doors are closed you need to break them down", 3000);
                return;
            }
            if (house.Locked && player.GetData<int>("HOUSEHEIST") == -1)
            {
                ...
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"The doors are closed! You weren't expected here!", 3000);
                return;
            }
            house.SendPlayer(player);
        ...
        private static void callback_housebuy(Player player, int act)
        {
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You don't have enough money to buy a house", 3000);
                return;
            }

            if (Houses.Count(h => h.Owner == player.Name) >= 1)
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You cannot buy more than one house", 3000);
                return;
            }
            ...
            if (vehicles > maxcars)
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"The house you are buying has {maxcars} parking lots, sell extra cars", 3000);
            ...
            Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Congratulations! you bought a house #{house.ID} for {house.Price}$", 3000);
            ...
            foreach (var v in NAPI.Pools.GetAllVehicles())
            {
                if (v.HasData("ACCESS") && v.GetData<string>("ACCESS") == "PERSONAL" && NAPI.Vehicle.GetVehicleNumberPlate(v) == vehicle)
                {
                    ...
                    Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"Your car was taken to the garage", 3000);
                    break;
                }
            }
            return;
        ...
        private static void callback_housebuy(Player player, Menu menu, Menu.Item item, string eventName, dynamic data)
        {
            switch (item.ID)
            {
                case "buy":
                    ...
                    if(house.GarageID == 0)
                    {
                        Notify.Error(player, "The house does not have a garage!", 3000);
                        return;
                    }
                    var maxcars = GarageManager.GarageTypes[GarageManager.Garages[house.GarageID].Type].MaxCars;
                    if (vehicles > maxcars)
                    ...
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You bought this house, don't forget to pay the tax for it at the ATM", 3000);
                    Notify.Send(player, NotifyType.Success, NotifyPosition.Center, $"DON'T FORGET TO PAY YOUR HOUSE TAXES AT YOUR NEAREST ATM!", 8000);
                    CheckAndKick(player);
                    house.SetLock(true);
                    house.SetOwner(player);
                    house.SendPlayer(player);
        ...
        public static void OpenHouseManageMenu(Player player)
        {
            ...
            menuItem.Text = "kick everyone out";
            menu.Add(menuItem);
            ...
            menuItem.Text = $"Sell a state for {Convert.ToInt32(price)}$";
            menu.Add(menuItem);
        ...
        private static void callback_housemanage(Player player, Menu menu, Menu.Item item, string eventName, dynamic data)
        {
            ...
            switch (item.ID)
            {
                case "changestate":
                    house.SetLock(!house.Locked);
                    if (house.Locked) Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"you closed the house", 3000);
                    else Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"you opened a house", 3000);
                    return;
                case "removeall":
                    house.RemoveAllPlayers(player);
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You kicked everyone out of the house", 3000);
                ...
                case "sell":
                    ...
                    MenuManager.Close(player);
                    Trigger.ClientEvent(player, "openDialog", "HOUSE_SELL_TOGOV", $"Do you really want to sell your house for ${price}?");
                    return;
                case "cars":
                    OpenCarsMenu(player);
                    return;
        ...
        public static void acceptHouseSellToGov(Player player)
        {
            ...
            Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You sold your house to the state for {price}$", 3000);
        }
        ...
        public static void OpenCarsSellMenu(Player player)
        {
            ...
            menuItem.Text = "Choose a car you want to sell";
            menu.Add(menuItem);
        ...
        private static void callback_carsell(Player player, Menu menu, Menu.Item item, string eventName, dynamic data)
        {
            ...
            Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"you sold {VehicleHandlers.VehiclesName.GetRealVehicleName(vData.Model)} ({item.ID}) for {price}$", 3000);
        ...
        public static void OpenFurnitureMenu(Player player)
        {
            ...
            menuItem.Text = "Furniture management";
            menu.Add(menuItem);
        ...
        private static void callback_furniture0(Player player, Menu menu, Menu.Item item, string eventName, dynamic data)
        {
            ...
            if (Main.Players[player].InsideHouseID == -1)
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You must be at home for this activity.", 3000);
                MenuManager.Close(player);
                return;
            }
            House house = GetHouse(player, true);
            ...
            if (item.ID == "tofurniture")
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You don't have furniture", 3000);
                    MenuManager.Close(player);
                    return;
                }
            ...
            else if (item.ID == "buyfurniture")
            {
                ...
                Menu.Item menuItem = new Menu.Item("header", Menu.MenuItem.Header);
                menuItem.Text = "Buying furniture";
                nmenu.Add(menuItem);

                menuItem = new Menu.Item("buy1", Menu.MenuItem.Button);
                menuItem.Text = "Gun safe (15000$)";
                nmenu.Add(menuItem);
                ...
                menuItem.Text = "Wardrobe with items (15000$)";
                nmenu.Add(menuItem);
        ...
        private static void callback_furniture1(Player player, Menu menu, Menu.Item item, string eventName, dynamic data)
        {
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "There is already too much furniture in your apartment, sell something", 3000);
                return;
            }
            if (item.ID == "buy1")
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "You don't have enough money to buy this furniture.", 3000);
                    return;
                }
            ...
            Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, "Congratulations on your successful furniture purchase!", 3000);
            MenuManager.Close(player);
        }
        ...
        private static void callback_furniture(Player player, Menu menu, Menu.Item item, string eventName, dynamic data)
        {
            ...
            if (item.ID == "sellit")
            {
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You have successfully sold {f.Name} for 7500$", 3000);
        ...
        public static void OpenRoommatesMenu(Player player)
        {
            ...
            {
                menuItem = new Menu.Item("label", Menu.MenuItem.Card);
                menuItem.Text = "No one is in your house";
                menu.Add(menuItem);
            }
        ...
        private static void callback_roommates(Player player, Menu menu, Menu.Item item, string eventName, dynamic data)
        {
            ...
            Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"you evicted {mName} from your home", 3000);
        }
        ...
        public static void OpenSelectedCarMenu(Player player, string number)
        {
            ...
            menuItem.Text = $"Reestablish {VehicleManager.VehicleRepairPrice[vClass]}$";
            menu.Add(menuItem);
        ...
        private static void callback_selectedcar(Player player, Menu menu, Menu.Item item, string eventName, dynamic data)
        {
            ...
            switch (item.ID)
            {
                case "sell":
                    ...
                    Trigger.ClientEvent(player, "openDialog", "CAR_SELL_TOGOV", $"Do you really want to sell to the state {VehicleHandlers.VehiclesName.GetRealVehicleName(vData.Model)} ({menu.Items[0].Text}) for ${price}?");
                    MenuManager.Close(player);
                    return;
                case "repair":
                    ...
                    if (vData.Health > 0)
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "The car does not need to be restored", 3000);
                        return;
                    }
                    ...
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"you restored {VehicleHandlers.VehiclesName.GetRealVehicleName(vData.Model)} ({menu.Items[0].Text})", 3000);
                    return;
                case "evac":
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Insufficient funds (not enough {200 - Main.Players[player].Money}$)", 3000);
                        return;
                    }
                ...
                case "spawnmycar":
                    ...
                    if (!check)
                    {
                        if (number != null)
                        {
                            Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"You have called a transport, wait for it within one second", 3000);
                            NAPI.Task.Run(() =>
                            {
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Hmm... This car is not in the garage", 3000);
                        return;
                    }
                    return;
                ...
                case "evac_pos":
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "The car does not need to be evacuated", 3000);
                        return;
                    }
                    ...
                    garage = GarageManager.Garages[GetHouse(player).GarageID];
                    garage.SendVehicleIntoGarage(number);
                    Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"Your car has been towed to a garage", 3000);
                    return;
                case "gps":
                    if (!Main.Players.ContainsKey(player)) return;
                    ...
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, "Your car's location has been marked in the GPS", 3000);
                    return;
                case "key":
                    if (!Main.Players.ContainsKey(player)) return;
                    ...
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You received a car key with a number {menu.Items[0].Text}", 3000);
                    return;
                case "changekey":
                    if (!Main.Players.ContainsKey(player)) return;
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Changing locks costs $1000", 3000);
                        return;
                    }

                    VehicleManager.Vehicles[menu.Items[0].Text].KeyNum++;
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You changed the locks on your car {menu.Items[0].Text}. Now old keys cannot be used", 3000);
                    return;
                case "back":
        ...
        public static void InviteToRoom(Player player, Player guest)
        {
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You have the maximum number of players in your house", 3000);
                return;
            }
            ...
            Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"You offered the player ({guest.Value}) settle down with you", 3000);
        }
        ...
        public static void acceptRoomInvite(Player player)
        {
            ...
            if (house.Roommates.Count >= MaxRoommates[house.Type])
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"The maximum number of players lives in the house", 3000);
                return;
            }
            ...
            Notify.Send(owner, NotifyType.Info, NotifyPosition.BottomCenter, $"Player ({player.Value}) settled down with you", 3000);
            Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"You have joined a player ({owner.Value})", 3000);
        }
        ...
        public static void InvitePlayerToHouse(Player player, Player guest)
        {
            ...
            Notify.Send(guest, NotifyType.Info, NotifyPosition.BottomCenter, $"Player ({player.Value})invited you to his house", 3000);
            Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"You invited a player ({guest.Value}) to your house", 3000);
        }
        ...
        public static void OfferHouseSell(Player player, Player target, int price)
        {
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You are too far away from the buyer", 3000);
                return;
            }
            House house = GetHouse(player, true);
            if (house == null)
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Too high/low price", 3000);
                return;
            }
        ...
        public static void acceptHouseSell(Player player)
        {
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Do you already have a home?", 3000);
                return;
            }
            ...
            Notify.Send(seller, NotifyType.Info, NotifyPosition.BottomCenter, $"Player ({player.Value}) bought your house", 3000);
            Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"You bought a house from a player ({seller.Value})", 3000);
        }
