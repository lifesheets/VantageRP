                    ...
                    case 0:
                        {// self inventory
                            ...
                            {//remove one item from player inventory
                                ...
                                {
                                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Do not throw away your documents", 3000);
                                    return;
                                }
                                else if (item.Type == ItemType.License)
                                {
                                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Can't throw away your licenses", 3000);
                                    return;
                                }
                                ...
                                else if (nInventory.ClothesItems.Contains(item.Type))
                                {
                                    if (item.IsActive)
                                    {
                                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "You must take off those clothes first.", 3000);
                                        return;
                                    }
                                ...
                                else if (nInventory.WeaponsItems.Contains(item.Type) || nInventory.MeleeWeaponsItems.Contains(item.Type) || item.Type == ItemType.StunGun)
                                {
                                    if (item.IsActive)
                                    {
                                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "You must take the weapon out of your hands", 3000);
                                        return;
                                    }
                                    items.RemoveAt(index);
                                    Items.onDrop(player, new nItem(item.Type, 1, item.Data), null);
