    ...
    class Sheriff : Script
    {
        ...
        public static void callSheriff(Player player, string reason)
        {
            NAPI.Task.Run(() =>
            {
                try {
                    ...
                    colshape.OnEntityExitColShape += (s, e) =>
                    {
                        if (e == player)
                        {
                            try
                            {
                                ...
                                Manager.sendFractionMessage(18, $"{e.Name.Replace('_', ' ')} canceled the call");
                    ...
                    Manager.sendFractionMessage(18, $"Received a call from a player ({player.Value}) - {reason}");
                    Manager.sendFractionMessage(18, $"~b~Received a call from a player ({player.Value}) - {reason}", true);
                    Manager.sendFractionMessage(9, $"Received a call from a player ({player.Value}) - {reason}");
                    Manager.sendFractionMessage(9, $"~b~Received a call from a player ({player.Value}) - {reason}", true);
                }
                catch { }
            });
        }
        ...
        public static void acceptCall(Player player, Player target)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "The player did not call the police or someone has already accepted this call", 3000);
                    return;
                }
                ...
                Manager.sendFractionMessage(18, $"{player.Name.Replace('_', ' ')} received a challenge from a player ({target.Value})");
                Manager.sendFractionMessage(18, $"~b~{player.Name.Replace('_', ' ')} received a challenge from a player ({target.Value})", true);
        ...
        public static void clearWantedLvl1(Player sender, params object[] arguments)
        {
            try
            {
                ...
                Notify.Send(sender, NotifyType.Success, NotifyPosition.BottomRight, $"You removed the wanted list from the passport holder {target}", 3000);
        ...
        public static void checkNumber1(Player sender, params object[] arguments)
        {
            try
            {
                ...
                {
                    Notify.Send(sender, NotifyType.Error, NotifyPosition.BottomCenter, $"Cars with this number were not found", 3000);
                    return;
                }
        ...
        public static void interactPressed(Player player, int interact)
        {
            ...
            switch (interact)
            {
                case 100:
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You are not a Sheriff officer", 3000);
                        return;
                    }
                ...
                case 110:
                    ...
                    else Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You are not a Sheriff officer", 3000);
                    return;
                case 120:
                    if (Main.Players[player].FractionID != 18)
                    {
                ...
                case 420:
                    ...
                    Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"You have been rewarded in 200$", 3000);
                    return;
                case 440:
                ...
                case 660:
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You must be in a work machine", 3000);
                        return;
                    }
                    Trigger.ClientEvent(player, "svem", 20, 20);
                    player.Vehicle.SetSharedData("BOOST", 20);
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Forgiven", 3000);
                    return;
            }
        }
        ...
        private static void callback_Sheriffclothes(Player client, Menu menu, Menu.Item item, string eventName, dynamic data)
        {
            switch (item.ID)
            {
                ...
                case "payday":
                    ...
                    {
                        Notify.Send(client, NotifyType.Success, NotifyPosition.BottomCenter, $"you started your work day", 3000);
                    ...
                    {
                        Notify.Send(client, NotifyType.Success, NotifyPosition.BottomCenter, $"you finished your work day", 3000); ;
