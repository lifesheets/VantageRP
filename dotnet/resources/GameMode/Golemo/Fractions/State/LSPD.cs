...
namespace Golemo.Fractions
{
    class Police : Script
    {
        ...
        public static void dropallneleagal(Player player)
        {
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You have no money bag or bag with a drill", 3000);
                return;
            }
        ...
        public static void interactPressed(Player player, int interact)
        {
            ...
            switch (interact)
            {
                case 10:
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"The warehouse is closed", 3000);
                        return;
                    }
                    if (!NAPI.Data.GetEntityData(player, "ON_DUTY"))
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You must start a working day", 3000);
                        return;
                    }
                ...
                case 11:
                    if (Main.Players[player].FractionID == 7)
                    {
                        ...
                        {
                            Trigger.ClientEvent(player, "OpenClothesChange", ("LSPD set #1", "LSPD set #2", "LSPD set #3", "LSPD set #4", "LSPD set #5", "LSPD set #6", "LSPD set #7", "LSPD set #8"), "LSPD", 7);
                            NAPI.Entity.SetEntityRotation(player, new Vector3(0, 0, 88));
                            NAPI.Entity.SetEntityDimension(player, (uint)(2000 + player.Value));
                        }
                    }
                    else Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You are not an employee LSPD", 3000);
                    return;
                case 12:
                    if (Main.Players[player].FractionID != 7)
                    {
                    ...
                    if (!NAPI.Data.GetEntityData(player, "ON_DUTY"))
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You must start a working dayь", 3000);
                        return;
                    }
                    if (!is_warg)
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"The emergency mode is not turned on", 3000);
                        return;
                    }
