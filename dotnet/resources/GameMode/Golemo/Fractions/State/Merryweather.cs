...
namespace Golemo.Fractions
{
    class Merryweather : Script
    {
        ...
        public static void interactPressed(Player player, int interact)
        {
            switch (interact)
            {
                ...
                case 855:
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "You are not in KillerBuro", 3000);
                        return;
                    }
        ...
        private static void callback_merycombat(Player client, Menu menu, Menu.Item item, string eventName, dynamic data)
        {
            ...
            {
                Notify.Send(client, NotifyType.Error, NotifyPosition.BottomCenter, $"You are not in KillerBuro", 3000);
                return;
            }
            if (!client.GetData<bool>("ON_DUTY"))
            {
