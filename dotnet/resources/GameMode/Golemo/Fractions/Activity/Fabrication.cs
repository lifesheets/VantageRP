        ...
        public static void Event_InteractPressed(Player player, int id)
        {
            switch (id)
            {
                ...
                case 56:
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You are not in {Fractions.Manager.getName(player.GetData<int>("CLUB"))}", 3000);
                        return;
                    }
                    ...
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, "You have unloaded all the material from the truck to the club's warehouse", 3000);
                    return;
                case 57:
                    if (!Main.Players.ContainsKey(player)) return;
