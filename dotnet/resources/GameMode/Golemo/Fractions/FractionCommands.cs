...
namespace Golemo.Fractions
{
    class FractionCommands : Script
    {
        ...
        public static void CMD_OpenFractionStock(Player player)
        {
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Warehouse is already open", 3000);
                return;
            }

            Stocks.fracStocks[Main.Players[player].FractionID].IsOpen = true;
            Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, "You opened a warehouse", 3000);
        }
        ...
        public static void CMD_CloseFractionStock(Player player)
        {
            ...
            if (!Stocks.fracStocks[Main.Players[player].FractionID].IsOpen)
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Warehouse already closed", 3000);
                return;
            }
        ...
        public static void SetFracRank(Player sender, Player target, int newrank)
        {
            ...
            {
                if(newrank <= 0) {
                    Notify.Send(sender, NotifyType.Error, NotifyPosition.BottomCenter, "Cannot set negative or zero rank", 3000);
                    return;
                }
                ...
                Notify.Send(sender, NotifyType.Warning, NotifyPosition.BottomCenter, $"You promoted a player {target.Name} to {Manager.Members[target].inFracName}", 3000);
        ...
        public static void InviteToFraction(Player sender, Player target)
        {
            if (Manager.canUseCommand(sender, "invite"))
            {
                ...
                {
                    Notify.Send(sender, NotifyType.Error, NotifyPosition.BottomCenter, $"The player is already in an organization", 3000);
                    return;
                }
                ...
                {
                    Notify.Send(sender, NotifyType.Error, NotifyPosition.BottomCenter, $"Unable to accept this player", 3000);
                    return;
                }
                ...
                Notify.Send(sender, NotifyType.Success, NotifyPosition.BottomCenter, $"You have been invited to a faction {target.Name}", 3000);
                Dashboard.sendStats(target);
            }
        }
        ...
        public static void UnInviteFromFraction(Player sender, Player target, bool mayor = false)
        {
            ...
            else
            {
                ...
                {
                    Notify.Send(sender, NotifyType.Error, NotifyPosition.BottomCenter, $"The player belongs to another organization", 3000);
                    return;
                }
            }
            ...
            Notify.Send(target, NotifyType.Warning, NotifyPosition.BottomCenter, $"You have been kicked out of a faction {Manager.FractionNames[Main.Players[sender].FractionID]}", 3000);
            Notify.Send(sender, NotifyType.Success, NotifyPosition.BottomCenter, $"You have been kicked out of a faction {target.Name}", 3000);
            Dashboard.sendStats(target);
            return;
        }
        ...
        public static void ticketToTarget(Player player, Player target, int sum, string reason)
        {
            ...
            if (sum > 7000)
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Limit on fine 7000$", 3000);
                return;
            }
            if (reason.Length > 100)
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Too big a reason", 3000);
                return;
            }
            ...
            Notify.Send(target, NotifyType.Success, NotifyPosition.BottomCenter, $"{player.Name} you have been fined {sum}$ for {reason}", 3000);
            Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You issued a fine for {target.Name} at the rate of {sum}$ for {reason}", 3000);

            Commands.RPChat("me", player, " issued a fine for {name}", target);
            Manager.sendFractionMessage(7, $"{player.Name} fined {target.Name} to the {sum}$ ({reason})", true);
        ...
        public static void arrestTarget(Player player, Player target)
        {
            ...
            if (player == target)
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Cannot be applied to self", 3000);
                return;
            }
            if (!NAPI.Data.GetEntityData(player, "ON_DUTY"))
            {
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"The player is already in jail", 3000);
                return;
            }
            ...
            Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You put a player ({target.Value}) to the {Main.Players[target].WantedLVL.Level * 20} minutes", 3000);
            ...
            Commands.RPChat("me", player, " placed {name} in the bullpen", target);
            Manager.sendFractionMessage(7, $"{player.Name} put in a bullpen {target.Name} ({Main.Players[target].WantedLVL.Reason})", true);
            Manager.sendFractionMessage(9, $"{player.Name} put in a bullpen {target.Name} ({Main.Players[target].WantedLVL.Reason})", true);
        ...
        public static void releasePlayerFromPrison(Player player, Player target)
        {
            ...
            if (Main.Players[target].ArrestTime == 0)
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Player not in jail", 3000);
                return;
            }
            freePlayer(target);
            Main.Players[target].ArrestTime = 0;
            Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You freed a player ({target.Value}) from jail", 3000);
            Notify.Send(target, NotifyType.Warning, NotifyPosition.BottomCenter, $"Player ({player.Value}) released you from prison", 3000);
            Commands.RPChat("me", player, " freed {name} from KPZ", target);
        }
        ...
        public static void freePlayer(Player player)
        {
            NAPI.Task.Run(() =>
            {
                try
                {
                    ...
                    Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, $"You have been released from prison", 3000);
                }
                catch { }
            });
        }
        ...
        public void ClientEvent_playerPressFollow(Player player)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"You released a player ({player.GetData<Player>("FOLLOWER").Value})", 3000);
        ...
        public static void targetFollowPlayer(Player player, Player target)
        {
            ...
            if (Manager.FractionTypes[fracid] == 2) // for gov factions
            {
                if (!NAPI.Data.GetEntityData(player, "ON_DUTY"))
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You must first start the work day", 3000);
                    return;
                }
            }
            ...
            if (NAPI.Data.HasEntityData(player, "FOLLOWER"))
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You are already dragging a player", 3000);
                return;
            }
            ...
            Commands.RPChat("me", player, "dragged {name} behind you", target);
            Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"You dragged a player along with you ({target.Value})", 3000);
        ...
        public static void targetUnFollowPlayer(Player player)
        {
            ...
            if (!NAPI.Data.HasEntityData(player, "FOLLOWER"))
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You don't drag anyone along", 3000);
                return;
            }
            Player target = NAPI.Data.GetEntityData(player, "FOLLOWER");
            unFollow(player, target);
            Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"You released a player ({target.Value})", 3000);
        ...
        public static void suPlayer(Player player, int pasport, int stars, string reason)
        {
            ...
            if (target == null)
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Passport holder must be online", 3000);
                return;
            }
            if (player != target)
            {
                ...
                if (Main.Players[target].ArrestTime != 0)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Jail player", 3000);
                    return;
                }
                ...
                {
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You declared a player " + target.Name.Replace('_', ' ') + " wanted", 3000);
            ...
            else Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You cannot put yourself on the wanted list", 3000);
        }
        ...
        public static void playerInCar(Player player, Player target)
        {
            ...
            if (player == target)
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Cannot be used on yourself", 3000);
                return;
            }
            var vehicle = VehicleManager.getNearestVehicle(player, 3);
            if (vehicle == null)
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"There are no cars nearby", 3000);
                return;
            }
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"There's no room in the car", 3000);
                return;
            }
            ...
            Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You pushed a player ({target.Value}) in the car", 3000);
            Notify.Send(target, NotifyType.Warning, NotifyPosition.BottomCenter, $"Player ({player.Value}) pushed you into the car", 3000);
        ...
        public static void playerOutCar(Player player, Player target)
        {
            if (player != target)
            {
                ...
                if (player.Position.DistanceTo(target.Position) < 3)
                {
                    if (NAPI.Player.IsPlayerInAnyVehicle(player))
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "You shouldn't be in the car", 2500);
                        return;
                    }
                    if (NAPI.Player.IsPlayerInAnyVehicle(target))
                    {
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You kicked a player ({target.Value}) out of the car", 3000);
                    ...
                    else Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"The player is not in the car", 3000);
                }
                else Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"The player is too far away from you", 3000);
            }
        ...
        public static void giveGunLic(Player player, Player target, int price)
        {
            ...
            Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"You offered to buy a weapon license for a player ({target.Value}) for ${price}", 3000);
