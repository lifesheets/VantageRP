    ...
    class Bus : Script
    {
            ...
            try
            {
                ...
                else
                {
                    if (NAPI.Data.GetEntityData(vehicle, "ON_WORK"))
                    {
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You do not have enough funds to pay for the fare", 3000);
                            VehicleManager.WarpPlayerOutOfVehicle(player);
                        }
                    }
                    else
        ...
        public static void acceptBusRent(Player player)
        {
            if (NAPI.Player.IsPlayerInAnyVehicle(player) && player.VehicleSeat == 0 && player.Vehicle.GetData<string>("TYPE") == "BUS")
            {
                ...
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"You have rented a bus. You have been assigned to a route {BusWaysNames[way]}", 3000);
            }
            else
            {
