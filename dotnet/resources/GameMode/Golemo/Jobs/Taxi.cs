    ...
    class Taxi : Script
    {
        ...
        public static void taxiRent(Player player)
        {
            if (NAPI.Player.IsPlayerInAnyVehicle(player) && player.VehicleSeat == 0 && player.Vehicle.GetData<string>("TYPE") == "TAXI")
            {
                if (player.Vehicle.GetData<Player>("DRIVER") == null)
                {
                    ...
                    Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"You rented a taxi. To offer the player to pay for the fare, write /tprice [ID] [Цена]", 3000);
        ...
        private static void order_onEntityExit(ColShape shape, Player player)
        {
            try
            {
                ...
                if (player.HasData("TAXI_DRIVER"))
                {
                    ...
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You left the taxi call", 3000);
                    try
                    {
        ...
        public void onPlayerEnterVehicleHandler(Player player, Vehicle vehicle, sbyte seatid)
        {
            try
            {
                ...
                if (seatid == 0)
                {
                    ...
                    if (Main.Players[player].WorkID == 3)
                    {
                        if (NAPI.Data.GetEntityData(player, "WORK") == null)
                        {
                            ...
                            {
                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You are missing " + (taxiRentCost - Main.Players[player].Money) + "$ to rent a taxi", 3000);
                                VehicleManager.WarpPlayerOutOfVehicle(player);
                            }
                        }
                        else if (NAPI.Data.GetEntityData(player, "WORK") == vehicle) NAPI.Data.SetEntityData(player, "IN_WORK_CAR", true);
                        else Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Are you already working", 3000);
                    }
                    else
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You don't work in a taxi. You can get a job at City Hall", 3000);
                        VehicleManager.WarpPlayerOutOfVehicle(player);
                    }
                }
                else
                {
                    ...
                    {
                        Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, "If you want to send your route to the driver, then put a mark on the map and press Z.", 5000);
            ...
            if (Main.Players[player].WorkID == 3)
            {
                if (NAPI.Data.GetEntityData(player, "WORK") != null)
                {
                    ...
                    if (NAPI.Data.GetEntityData(vehicle, "TYPE") == "TAXI")
                    {
                        ...
                        Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"You offered the player ({target.Value}) Pay for a trip for {price}$", 3000);
                    }
                }
        ...
        public static void acceptTaxi(Player player, Player target)
        {
            if (Main.Players[player].WorkID == 3 && NAPI.Data.GetEntityData(player, "WORK") != null)
            {
                if (player.HasData("PASSAGER"))
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You have already taken the order", 3000);
                    return;
                }
                if (NAPI.Data.GetEntityData(target, "IS_CALL_TAXI") && !target.HasData("TAXI_DRIVER"))
                {
                    Notify.Send(target, NotifyType.Warning, NotifyPosition.BottomCenter, $"A taxi driver ({player.Value}) Accepted your call. Stay in place", 3000);
                    Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"You accepted the player's call ({target.Value})", 3000);
