    ...
    public class Character : CharacterData
    {
            ...
            try
            {
                NAPI.Task.Run(() =>
                {
                    try
                    {
                        ...
                        if (IsBonused)
                        {
                            Trigger.ClientEvent(player, "UpdateLastBonus", $"YOU ALREADY RECEIVED A GIFT"); //todo lastbonus
                        }
                        else
                        {
                            ...
                            var hour = date.Hour;
                            var min = date.Minute;
                            Trigger.ClientEvent(player, "UpdateLastBonus", $"Time left for gift {hour}:{min}"); //todo lastbonus
                        }
                        #endregion
                ...
                if (Warns > 0 && DateTime.Now > Unwarn)
                {
                    ...
                    Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, $"One warning has been lifted. You have left {Warns}", 3000);
                }
        ...
        public async Task<int> Create(Player player, string firstName, string lastName)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Error in the length of the first / last name", 3000);
                    return -1;
                }
                if (Main.PlayerNames.ContainsValue($"{firstName}_{lastName}"))
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "This name is already taken", 3000);
                    return -1;
                }
