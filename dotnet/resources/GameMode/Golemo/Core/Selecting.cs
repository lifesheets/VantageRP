                    ...
                    case "MoneyBag":
                        {
                            ...
                            {
                                entity.SetSharedData("PICKEDT", false);
                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Do you already have a bag?", 3000);
                                return;
                            }
            ...
            try
            {
                ...
                switch (index)
                {
                    ...
                    case 1:
                        ...
                        else
                        {
                            ...
                            if (vehicle.HasData("ACCESS") && vehicle.GetData<string>("ACCESS") == "FRACTION" && vehicle.GetData<int>("FRACTION") != Main.Players[player].FractionID)
                            {
                                ...
                                {
                                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "You can't open the trunk of this car", 3000);
                                    return;
                                }
                            }
                            if (player.Position.DistanceTo(BonePos) > 2)
                            {
                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "You are too far from the trunk", 3000);
                                return;
                            }
                    ...
                    case 3:
                        if (player.IsInVehicle)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "You cannot open inventory while in a car", 3000);
                            return;
                        }
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "You cannot open the car inventory while the trunk is closed", 3000);
                            return;
                        }
                    ...
                    case 4:
                        ...
                        if (itempetrol == null)
                        {
                            Notify.Error(player, "You don't have a canister");
                            return;
                        }
                        if (!itempetrol.IsActive)
                        {
                            Notify.Error(player, "You do not have a canister in your hands");
                            return;
                        }
                        if (!vehicle.HasSharedData("PETROL"))
                        {
                            Notify.Error(player, "This vehicle cannot be refueled.");
                            return;
                        }
                        var fuel = vehicle.GetSharedData<int>("PETROL");
                        if (fuel == VehicleManager.VehicleTank[vehicle.Class])
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"The car has a full tank", 3000);
                            GUI.Dashboard.Close(player);
                            return;
                        }
                        fuel += 20;
                        Notify.Succ(player, $"You refueled the transport for 20 liters, now it's in the tank {fuel}");
                    ...
                    case 6:
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "This is not your car..", 3000);
                            return;
                        }
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "This vehicle does not have a number plate.", 3000);
                            return;
                        }
                        ...
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, "You took the numbers", 3000);
                        return;
                }
            ...
            try
            {
                ...
                switch (action)
                {
                    ...
                    case "Выдать лицензию на оружие":
                        ...
                        {
                            Notify.Send(target, NotifyType.Error, NotifyPosition.BottomCenter, $"You already have a gun license", 3000);
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Player({target.Value}) already have a gun license", 3000);
                            return;
                        }
                        Notify.Send(target, NotifyType.Success, NotifyPosition.BottomCenter, $"You got a gun license", 3000);
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You issued a weapon license to a Player ({target.Value})", 3000);
                    ...
                    case "Передать деньги":
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"The transfer of money is available after the first level", 3000);
                            return;
                        }
                    ...
                    case "Предложить обмен":
                        ...
                        Notify.Send(target, NotifyType.Warning, NotifyPosition.BottomCenter, $"Player ({player.Value}) invited you to exchange objects. Y/N - Accept/reject", 3000);
                        Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"You offered the player ({target.Value}) exchange objects.", 3000);
                        return;
                    case "Мешок":
                        if (player.IsInVehicle) return;
        ...
        public static void playerKissTarget(Player player, Player target)
        {
            if ((!player.HasData("CUFFED") && !player.HasSharedData("InDeath")) || player.HasData("CUFFED") && player.GetData<bool>("CUFFED") == false && player.HasSharedData("InDeath") && player.GetSharedData<bool>("InDeath") == false)
            {
                if ((!target.HasData("CUFFED") && !target.HasSharedData("InDeath")) || target.HasData("CUFFED") && target.GetData<bool>("CUFFED") == false && target.HasSharedData("InDeath") && target.GetSharedData<bool>("InDeath") == false)
                {
                    ...
                    target.SetData("IS_REQUESTED", true);
                    Notify.Send(target, NotifyType.Warning, NotifyPosition.BottomCenter, $"({player.Value}) He wants to kiss you. Y/N - agree/refuse", 3000);
                    //Main.OpenAnswer(target, $"Гражданин ({player.Name}) хочет поцеловать Вас");
                    Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"You suggested ({target.Value}) kiss.", 3000);
                }
                else Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Does not want to kiss", 3000);
            }
            else Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Cannot kiss", 3000);
        }
        ...
        public static void playerHandshakeTarget(Player player, Player target)
        {
            if ((!player.HasData("CUFFED") && !player.HasSharedData("InDeath")) || player.HasData("CUFFED") && player.GetData<bool>("CUFFED") == false && player.HasSharedData("InDeath") && player.GetSharedData<bool>("InDeath") == false)
            {
                if ((!target.HasData("CUFFED") && !target.HasSharedData("InDeath")) || target.HasData("CUFFED") && target.GetData<bool>("CUFFED") == false && target.HasSharedData("InDeath") && target.GetSharedData<bool>("InDeath") == false)
                {
                    ...
                    Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"You offered the player ({target.Value}) shake hands.", 3000);
                }
                else Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "It is impossible to shake the hand of the player at the moment", 3000);
            }
            else Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "It is impossible to shake the hand of the player at the moment", 3000);
        }
