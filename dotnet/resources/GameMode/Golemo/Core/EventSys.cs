            ...
            if (Main.Players.ContainsKey(player))
            {
                ...
                if (AdminEvent.EventState == 0)
                {
                    if (eventname.Length < 50)
                    {
                        ...
                        AddAdminEventLog();
                        NAPI.Chat.SendChatMessageToAll("!{#A87C33}Dear players, the event will start soon '" + eventname + "'!");
                        if (members != 0) NAPI.Chat.SendChatMessageToAll("!{#A87C33}There is a participant limit for this event.: " + members + ".");
                        else NAPI.Chat.SendChatMessageToAll("!{#A87C33}There is no participant limit for this event..");
                        if (AdminEvent.Zone != null) NAPI.Chat.SendChatMessageToAll("!{#A87C33}The event is valid in the zone " + radius + "m from teleport point.");
                        NAPI.Chat.SendChatMessageToAll("!{#A87C33}To teleport to an event, enter the /mp command");
                    }
                    else player.SendChatMessage("The name of the event is too long, come up with a shorter one.");
                }
        ...
        public void StartEvent(Player player)
        {
            if (Main.Players.ContainsKey(player))
            {
                ...
                if (AdminEvent.EventState == 1)
                {
                    ...
                    else player.SendChatMessage("Unable to start an event without attendees.");
                }
                else player.SendChatMessage("The event has either not been created or is already running.");
            }
        }
