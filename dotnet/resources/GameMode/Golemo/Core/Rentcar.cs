...
namespace Golemo.Core
{
    class Rentcar : Script
    {
        ...
        public static void OnResourceStart()
        {
            try
            {
                ...
                Log.Write($"Loaded {_rentAreas.Count} vehicle rental sites");
            }
            catch (Exception e)
            {
        ...
        public void Event_OnPlayerEnterVehicle(Player player, Vehicle vehicle, sbyte seatid)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "This vehicle is already rented", 3000);
        ...
        public static void CMD_CreateBlipForRentedCar(Player player)
        {
            ...
            {
                Notify.Alert(player, "You don't have a rental vehicle");
                return;
            }
        ...
        public static void RentCar(Player player, string vname, int minutes = 10)
        {
            ...
            {
                Notify.Error(player, "Do you already have a rental vehicle?", 4500);
                return;
            }
            ...
            {
                Notify.Error(player, "Insufficient funds", 3500);
                return;
            }
        ...
        public static void CheckRentCarTime(Player player)
        {
            try
            {
                if (player.HasData("RENTED_CAR") && player.HasData("RENTED_TIME"))
                {
                    ...
                    {
                        Notify.Alert(player, $"Through {MINUTE_WHEN_SEND_NOTIFY_FOR_RENT_TIME} minutes the rental time will end", 6000);
                    }
                    if (player.GetData<DateTime>("RENTED_TIME") < DateTime.Now)
                    {
                        Notify.Alert(player, "The rental time of your vehicle has ended, the vehicle will be returned", 5000);
    ...
    class RentArea
    {
        ...
        {
            { RentAreaType.Boats, "Boat rental" },
            { RentAreaType.Helicopters, "Helicopter rental" },
            { RentAreaType.Motorcycles, "Transport rental" },
            { RentAreaType.Cycles, "Transport rental" },
            { RentAreaType.Planes, "Aircraft rental" },
            { RentAreaType.Sedans, "Transport rental" },
            { RentAreaType.Sports, "Transport rental" },
            { RentAreaType.OffRoad, "Transport rental" },
        };
        #endregion
    }
}
