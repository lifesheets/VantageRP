    ...
    public class Account : AccountData
    {
        ...
        public async Task<LoginEvent> LoginIn(Player client, string login_, string pass_)
        {
            try
            {
                ...
                if (client != null && !Main.Accounts.ContainsKey(client) && !Main.LoggedIn.ContainsKey(Login))
                {
                    Main.LoggedIn.Add(Login, client);
                    Main.Accounts.Add(client, this);
                }
                else
                {
                    ...
                    Notify.Send(client, NotifyType.Error, NotifyPosition.Center, $"Вы сейчас будете кикнуты. Попробуйте заново войти.", 500);
                    NAPI.Task.Run(() =>
                    {
                        try
                        {
                        ...
                        catch (Exception e) { Log.Write("NAPI.Task.Run: " + e.ToString(), nLog.Type.Error); }
                    }, delayTime: 500);
                }
