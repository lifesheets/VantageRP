...
namespace Golemo.Core
{
    class BusinessManager : Script
    {
        ...
        public static void interactionPressed(Player player)
        {
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Someone is dragging you along.", 3000);
                return;
            }
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"The {BusinessTypeNames[biz.Type]} not working at the moment", 3000);
                return;
            }
            ...
            switch (biz.Type)
            {
                ...
                case 17:
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Let go man", 3000);
                        return;
                    }
                ...
                case 7:
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You must finish your work day.", 3000);
                        return;
                    }
                ...
                case 12:
                    ...
                    if (!veh.HasData("ACCESS") && (veh.GetData<string>("ACCESS") != "PERSONAL" || veh.GetData<string>("ACCESS") != "GARAGE"))
                    {
                        ...
                        if (!access)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "You can only tune personal cars", 3000);
                            return;
                        }
                    }
                    if (player.Vehicle.Class == 13)
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "The bike cannot be tuned", 3000);
                        return;
                    }
        ...
        public static void RemoteEvent_petshopBuy(Player player, string petName)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Your place of residence is not suitable for pets", 3000);
                    return;
                }
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Unfortunately, there are no pets of this kind in the store yet.", 3000);
                    return;
                }
                ...
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"You are now a happy owner {petName}!", 3000);
            }
                ...
                if (player.IsInVehicle)
                {
                    if (player.VehicleSeat == 0)
                    {
                        if (VehicleStreaming.GetVehicleDirt(player.Vehicle) >= 0f)
                        {
                            ...
                            Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, "Your vehicle has been washed.", 3000);
                        }
                        else
                            Notify.Send(player, NotifyType.Alert, NotifyPosition.BottomCenter, "Your vehicle is not dirty.", 3000);
                    }
                    else
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Only the driver can wash the vehicle.", 3000);
                }
                return;
            }
            catch (Exception e)
        ...
        public static void RemoteEvent_tuningSeatsCheck(Player player)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You must be in your personal car", 3000);
                    return;
                }
                if (player.Vehicle.Class == 13)
                {
        ...
        public static void RemoteEvent_buyTuning(Player player, params object[] arguments)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You still lack {price - Main.Players[player].Money}$ to purchase this modification", 3000);
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "This auto repair shop has run out of parts.", 3000);
                    Trigger.ClientEvent(player, "tunBuySuccess", -2);
                    return;
                }
        ...
        public static void RemoteEvent_buyMasks(Player player, int variation, int texture)
        {
            try
            {
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, "You have bought a new mask. It has been added to your inventory..", 3000);
                return;
            }
        ...
        public static void RemoteEvent_buyTattoo(Player player, params object[] arguments)
        {
            try
            {
                ...
                //{
                //    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "This tattoo parlor cannot provide this service", 3000);
                //    return;
                //}
                ...
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"You got a tattoo {tattoo.Name} for {Convert.ToInt32(price)}$", 3000);
            }
        ...
        public static void RemoteEvent_buyBarber(Player player, string id, int style, int color)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Only available for female characters", 3000);
                    return;
                }
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "This barbershop is unable to provide this service at the moment", 3000);
                    return;
                }
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You paid for the Barber Shop service ({Convert.ToInt32(price)}$)", 3000);
                return;
            }
        ...
        public static void fillCar(Player player, int lvl)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"This type of fuel is not for your car", 3000);
                    return;
                }
                ...
                if (isGov)
                {
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"To refuel transport for the state. account, you must be in the state. organizations", 3000);
                        return;
                    }
                    if (!vehicle.HasData("ACCESS") || vehicle.GetData<string>("ACCESS") != "FRACTION" || vehicle.GetData<int>("FRACTION") != frac)
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You cannot refuel non-state transport at public expense", 3000);
                        return;
                    }
                    if (Fractions.Stocks.fracStocks[frac].FuelLeft < lvl * biz.Products[0].Price)
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"State refueling limit transport per day exhausted", 3000);
                        return;
                    }
                }
                else
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Left at the gas station {biz.Products[0].Lefts}l", 3000);
                    return;
                }
                if (isGov)
                {
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Transport refueled", 3000);
        ...
        public static void fillCarDiesel(Player player, int lvl)
        {
            try
            {
                ...
                if (!takeProd(biz.ID, lvl, "Бензин", lvl * biz.Products[0].Price))
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Left at the gas station {biz.Products[0].Lefts}л", 3000);
                    return;
                }
                if (isGov)
                {
        ...
        public static void bizNewPrice(Player player, int price, int BizID)
        {
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Can't set the price", 3000);
            ...
            foreach (var p in biz.Products)
            {
                if (p.Name == prodName)
                {
                    ...
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Now {p.Name} costs {p.Price}{ch}", 3000);
        ...
        public static void bizOrder(Player player, int amount, int BizID)
        {
            ...
            foreach (var p in biz.Products)
            {
                if (p.Name == player.GetData<string>("SELECTPROD"))
                {
                    if (p.Ordered)
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You have already ordered this item", 3000);
                        OpenBizProductsMenu(player);
                        return;
                    }
                    ...
                    if (biz.Type >= 2 && biz.Type <= 5)
                    {
                        if (amount > 3)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Enter a value from 1 to 3", 3000);
                            OpenBizProductsMenu(player);
                            return;
                        }
                    }
                    else if (biz.Type == 14)
                    {
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Enter a value from 1 to {ProductsCapacity[p.Name] - p.Lefts}", 3000);
                            OpenBizProductsMenu(player);
                            return;
                        }
                    }
                    else
                    {
                        ...
                        {
                            var text = "";
                            if (ProductsCapacity[p.Name] - p.Lefts < 10) text = "Do you have enough items in stock?";
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Insufficient funds on the account", 3000);
                        return;
                    }
                    ...
                    player.SendChatMessage($"Your order number: {order.UID}");
                    return;
                }
            }
        }
        ...
        public static void SetTimerToAddProducts(Player player, Business biz, Order order)
        {
            ...
            try
            {
                Timers.StartOnce($"{Main.Players[pl].UUID}_ordertime_{order.UID}", 180000, () => { //3  minutes 180.000
                    ...
                    if (ow != null)
                        Notify.Alert(ow, $"Your order for{order.Name} It was performed", 3000);
        ...
        public static void buyBusinessCommand(Player player)
        {
            ...
            {
                Notify.Error(player, "You must be near the business");
                return;
            }
            ...
            {
                Notify.Error(player, $"You cannot purchase more {Group.GroupMaxBusinesses[Main.Accounts[player].VipLvl]} businesses");
                return;
            }
            if (biz.Type == 2)
            {
                Notify.Error(player, "This business is not available for purchase");
                return;
            }
            if (biz.Owner == "Государство")
            {
                ...
                {
                    Notify.Error(player, "You do not have enough funds");
                    return;
                }
                GameLog.Money($"player({Main.Players[player].UUID})", $"server", biz.SellPrice, $"buyBiz({biz.ID})");
                Notify.Succ(player, $"Congratulations! You bought {BusinessManager.BusinessTypeNames[biz.Type]}, do not forget to pay the tax for it at the ATM");
                biz.Owner = player.Name.ToString();
            }
            else if (biz.Owner == player.Name.ToString())
            {
                Notify.Error(player, "This business belongs to you");
                return;
            }
            else if (biz.Owner != player.Name.ToString())
            {
                Notify.Error(player, "This business belongs to another player");
                return;
            }
        ...
        public static void createBusinessUnloadpoint(Player player, int bizid)
        {
            ...
            Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Successfully created unloading point for business ID: {bizid}", 3000);
        }
        ...
        public static void deleteBusinessCommand(Player player, int id)
        {
            ...
            if(biz == null)
            {
                Notify.Error(player, "You entered a non-existent business ID", 4500);
                return;
            }
            if (biz.Type == 6)
            {
            ...
            Notify.Succ(player, $"You have deleted the business №{id}", 3000);
        }
        ...
        public static void sellBusinessCommand(Player player, Player target, int price)
        {
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"player too far", 3000);
                return;
            }

            if (Main.Players[player].BizIDs.Count == 0)
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You don't have a business", 3000);
                return;
            }

            if (Main.Players[target].BizIDs.Count >= Group.GroupMaxBusinesses[Main.Accounts[target].VipLvl])
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"The player bought the maximum number of businesses", 3000);
                return;
            }
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You can't sell the business for that price. Set price from {biz.SellPrice / 2}$ to {biz.SellPrice * 3}$", 3000);
                return;
            }
            ...
            Trigger.ClientEvent(target, "openDialog", "BUSINESS_BUY", $"{player.Name} offered to buy {BusinessTypeNames[biz.Type]} for ${price}");
            target.SetData("SELLER", player);
            target.SetData("SELLPRICE", price);
            target.SetData("SELLBIZID", biz.ID);

            Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"You offered the player ({target.Value}) buy your business for {price}$", 3000);
        }
        ...
        public static void acceptBuyBusiness(Player player)
        {
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"The business no longer belongs to the player", 3000);
                return;
            }

            if (Main.Players[player].BizIDs.Count >= Group.GroupMaxBusinesses[Main.Accounts[player].VipLvl])
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You have the maximum number of businesses", 3000);
                return;
            }
            ...
            Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"you bought from {seller.Name.Replace('_', ' ')} {BusinessTypeNames[biz.Type]} for {price}$", 3000);
        ...
        public static void OpenBizListMenu(Player player)
        {
            if (Main.Players[player].BizIDs.Count == 0)
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You don't have any business", 3000);
                return;
            }
        ...
        private static void callback_bizsell(Player client, Menu menu, Menu.Item item, string eventName, dynamic data)
        {
            ...
            switch (item.ID)
            {
                case "govsell":
                    ...
                    Notify.Send(client, NotifyType.Success, NotifyPosition.BottomCenter, $"You sold the business to the state for {price}$", 3000);
                    MenuManager.Close(client);
                    return;
                case "back":
                    MenuManager.Close(client);
        ...
        private static void callback_bizsetting(Player client, Menu menu, Menu.Item item, string eventName, dynamic data)
        {
            ...
            switch (item.ID)
            {
                case "setprice":
                    ...
                    {
                        Notify.Send(client, NotifyType.Error, NotifyPosition.BottomCenter, $"Unable to set price for this item", 3000);
                        return;
                    }
                ...
                case "order":
                    ...
                    {
                        Notify.Send(client, NotifyType.Error, NotifyPosition.BottomCenter, $"If you want to resume selling services, order consumables", 3000);
                        return;
                    }
                ...
                case "cancel":
                    ...
                    foreach (var p in biz.Products)
                    {
                        ...
                        if (p.Ordered)
                        {
                            ...
                            if (order.Taked)
                            {
                                Notify.Send(client, NotifyType.Error, NotifyPosition.BottomCenter, $"You cannot cancel an order while it is being delivered.", 3000);
                                return;
                            }
                            biz.Orders.Remove(order);
                            p.Ordered = false;
        ...
        */
        ...
        public static void Event_ShopCallback(Player client, int index)
        {
            try
            {
                ...
                if (prod.Name == "Сим-карта")
                {
                    ...
                    {
                        Notify.Send(client, NotifyType.Error, NotifyPosition.BottomCenter, "Not enough goods in stock", 3000);
                        return;
                    }
                ...
                else
                {
                    ...
                    if (type != -1)
                    {
                        ...
                        if (tryAdd == -1 || tryAdd > 0)
                        {
                            Notify.Send(client, NotifyType.Error, NotifyPosition.BottomCenter, $"Your inventory can no longer hold {prod.Name}", 3000);
                            return;
                        }
                        else
                        {
        ...
        public static void Event_WShopAmmo(Player client, string text1, string text2)
        {
            try
            {
                ...
                {
                    Notify.Send(client, NotifyType.Error, NotifyPosition.BottomCenter, $"You don't have a gun license", 3000);
                    return;
                }

                if (ammo == 0)
                {
                    Notify.Send(client, NotifyType.Error, NotifyPosition.BottomCenter, $"You did not specify the number of cartridges", 3000);
                    return;
                }
                ...
                Notify.Send(client, NotifyType.Success, NotifyPosition.BottomCenter, $"You bought {nInventory.ItemsNames[(int)AmmoTypes[category]]} x{ammo} for {totalPrice}$", 3000);
                return;
            }
