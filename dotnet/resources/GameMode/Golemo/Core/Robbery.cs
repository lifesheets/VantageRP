...
namespace Golemo.Core
{
        ...
        private static void callback_safedoor(Player player, Menu menu, Menu.Item item, string eventName, dynamic data)
        {
            switch (item.ID)
            {
                case "change":
                    ...
                    else Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You can't do it", 3000);
                    return;
                case "crack":
                    MenuManager.Close(player);
    ...
    public class Safe
    {
        ...
        public void Loot(Player player)
        {
            ...
            if (SafeLoot == 0)
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"There's no more money in the safe", 3000);
                return;
            }
            ...
            else
            {
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Did you take the bag with {money}$", 3000);
            }
