    ...
    class LuckyWheel : Script
    {
        ...
        private static void ComeToLuckyWheel(Player player)
        {
            if (DateTime.Now < WaitFor)
            {
                ...
                player.SendNotification("You have to wait a little");
                return;
            }
            else if (!MoneySystem.Wallet.ChangeLuckyWheelSpins(player, -1))
            {
                Notify.Error(player, "You don't have enough scroll attempts!");
                return;
            }
            else
            {
        ...
        private static void FinishSpin(Player player)
        {
            if (player.HasSharedData("LUCKY_WHEEL_WIN") && player.HasSharedData("LUCKY_WHEEL_CALL"))
            {
                ...
                Notify.Succ(player, $"win: {resultName}. Congratulations!");
        ...
        private static void GiveOutPrizeWeapon(Player player)
        {
            ...
            switch (randomInt)
            {
                case 0:
                    ...
                    {
                        Notify.Alert(player, $"Not enough space, you've been compensated {amountCompensation}$");
        ...
        private static void GiveOutPrizeVehicle(Player player)
        {
            ...
            {
                MoneySystem.Wallet.Change(player, amountCompensation);
                Notify.Alert(player, $"You received compensation {amountCompensation}$ because you have the maximum number of cars");
            }
            else
            {
                ...
                Notify.Succ(player, $"You have received a unique car {VehicleHandlers.VehiclesName.GetRealVehicleName(model)}.");
            }
        }
