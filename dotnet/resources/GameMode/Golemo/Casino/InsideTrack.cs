...
namespace Golemo.Core
{
    class InsideTrack : Script
    {
        ...
        public static void Payout()
        {
            foreach(ITBet bet in Bets)
            {
                if (bet.Horse != WinnerHorse)
                {
                    try
                    {
                        Notify.Send(bet.Pl, NotifyType.Info, NotifyPosition.BottomCenter, $"Your bet is lost", 3000);
                    }
                    catch { }
                    continue;
                }
                ...
                try
                {
                    ...
                    Notify.Send(bet.Pl, NotifyType.Success, NotifyPosition.BottomCenter, $"Your horse has won {bet.Payout}", 3000);
        ...
        public static void AddBet(Player player, int horse, int bet)
        {
            ...
            Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Bet accepted on horse #{horse}", 3000);
        }
        ...
        public static void WinnerBet(Player player, int bet)
        {
            ...
            Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Your horse has won. win {bet * 2}", 3000);
