...
namespace Golemo.Casino
{
    class BlackJack : Script
    {
        ...
        public static void Bet(Player player)
        {
            ...
            Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"You have placed a bet {chips} chips", 3000);
                    ...
                    if (BlackJackBetTime[tb] != 0)
                    {
                        if (BlackJackSeatsPlayers[tb].Count != 0)
                        {
                            for (int i = 0; i < BlackJackSeatsPlayers[tb].Count; i++)
                            {
                               ...
                               // BlackJackSeatsPlayers[tb][i].SendChatMessage($"Waiting for bets [ {BlackJackBetTime[tb]} ]");
                            ...
                            try
                            {
                                if (BlackJackPlayers[tb][i].GetData<int>("BJ_ANSWER") == 0)
                                {
                                    ...
                                    else if (res == 1)
                                    {
                                        ...
                                        Notify.Send(BlackJackPlayers[tb][i], NotifyType.Info, NotifyPosition.BottomCenter, "You have blackjack", 3000);
                                        BlackJackPlayers[tb][i].SetData("BJ_ANSWER", 0);
        ...
        public static void CheckFinalSum(Player player)
        {
            ...
            {
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"You lost {bet} chips", 3000);
            ...
            {
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"you won {bet * 2} chips", 3000);
            ...
            {
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"Draw", 3000);
